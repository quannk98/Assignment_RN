...
function item_listviewPostShare(props) {

    const [like, setLike] = useState(true);
    const [comment, setComment] = useState(false);
    const { dataProfileshare } = props;
    function clickLike() {
        if (like == true) {
            setLike(false)
        }
    ...
    return (
        <View style ={{backgroundColor:"#CCCCCC",  margin: 10,
        borderRadius: 10,}}>
            <View>
            <View style={styles.Viewname}>
                    <Image style={{ width: 60,height: 60,borderRadius: 60,marginStart: 15,marginEnd: 20}} source={require('E:/React Native/Assignment/Img/putin.jpg')}
                    />
                    <Text style={styles.textname}>{dataProfileshare.name} đã chia sẻ bài viết</Text>
                </View>
                <View style={styles.container}>
                <View style={styles.Viewname}>
                    <Image style={styles.imgavtar} source={require('E:/React Native/Assignment/Img/putin.jpg')}
                    />
                    <Text style={styles.textname}>{dataProfileshare.name}</Text>
                </View>
                <View>
                    <Text style={styles.texttitle}>{dataProfileshare.title}</Text>
                </View>
                <View style={{ alignItems: 'center' }}>
                    <Image style={styles.imgcontent} source={{ uri: dataProfileshare.image }}
                    />
                </View>
...
export default item_listviewPostShare;
